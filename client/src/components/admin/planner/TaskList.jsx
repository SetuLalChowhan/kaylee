...
const TaskListItem = ({ task, onEdit, onDelete, onToggle }) => {
  ...
  return (
    <div className="flex items-center justify-between p-6 hover:bg-gray-50 transition-all group border-b border-gray-50 last:border-b-0">
      <div className="flex items-center gap-6">
        ...
        >
          {task.completed ? (
            ...
            <Circle className="w-6 h-6 text-gray-200 hover:text-Primary transition-colors" />
          )}
        </button>
        <div>
          <h4 className={`text-base font-bold ${task.completed ? 'text-gray-400 line-through' : 'text-[#1A1A1A]'}`}>
            {task.name}
          </h4>
...
const TaskList = ({ tasks, onEdit, onDelete, onToggle }) => {
  return (
    <div className="bg-white rounded-[32px] border border-gray-50 shadow-sm mb-10 relative">
      <div className="p-8 border-b border-gray-50 rounded-t-[32px]">
        <h2 className="text-xl font-bold text-[#1A1A1A]">All task this week</h2>
      </div>
      ...
      <div className="flex flex-col">
        {tasks.length > 0 ? (
          ...
          <div className="p-20 text-center text-gray-400 font-medium rounded-b-[32px]">
