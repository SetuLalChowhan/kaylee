 ...
 */
export const getPublicPortfolio = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { slug } = req.params as { slug: string };

  if (!slug) return next(new AppError("User slug/displayName is required", 400));
