...
export const forgotPassword = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  res.status(200).json({ status: "success", message: "Reset OTP sent to your email." });
});
...
export const resendOtp = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const subject = type === "FORGOT_PASSWORD" ? "Password Reset OTP" : "Account Verification OTP";
...
export const verifyResetOtp = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!user || user.verificationOtp !== otp) return next(new AppError("Invalid OTP", 400));
  if (user.otpExpires && new Date() > user.otpExpires) return next(new AppError("OTP has expired", 400));
...
export const resetPassword = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  res.status(200).json({ status: "success", message: "Password reset successful! You can now login." });
});
