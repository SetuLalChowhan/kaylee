...
export async function seedPlans() {
  ...
  let standardPriceId: string | null = null;
  ...
  if (process.env.STRIPE_SECRET_KEY) {
    try {
      ...
      let foundingProd = products.data.find(
        (p) => p.name.toLowerCase() === "founding member" || p.name.toLowerCase() === "founding member subscription"
      );
      if (!foundingProd) {
      ...
      let foundingPrice = foundingPrices.data.find(
        (p) => p.unit_amount === 1999 && p.currency === "aud" && p.recurring?.interval === "month"
      );
      if (!foundingPrice) {
      ...
      // 2. Standard Product
      let standardProd = products.data.find(
        (p) => p.name.toLowerCase() === "standard" || p.name.toLowerCase() === "standard subscription"
      );
      if (!standardProd) {
      ...
      let standardPrice = standardPrices.data.find(
        (p) => p.unit_amount === 2999 && p.currency === "aud" && p.recurring?.interval === "month"
      );
      if (!standardPrice) {
